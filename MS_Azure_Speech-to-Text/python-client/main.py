...
root_dir = Path.home().joinpath('Dropbox','DATASETS_AUDIO','AOLME_SD_Collection','TestSet','Commercial_results')
output_folder_json_pth = root_dir.joinpath('json_raw')

logging.basicConfig(stream=sys.stdout, level=logging.DEBUG,
        format="%(asctime)s %(message)s", datefmt="%m/%d/%Y %I:%M:%S %p %Z")

# Your subscription key and region for the speech service
SUBSCRIPTION_KEY = "769ed07f4cc845c6b49bd5db3e3dc05a"
SERVICE_REGION = "southcentralus"

NAME = "sdminitest1"
DESCRIPTION = "part 1 of speaker diarization veronica's GT"

LOCALE = "en-US"
RECORDINGS_BLOB_URI = "https://longaudiosaolme.blob.core.windows.net/minutesaudios/jenny_W_todos_1.wav"

# Provide the uri of a container with audio files for transcribing all of them with a single request
RECORDINGS_CONTAINER_URI = "https://longaudiosaolme.blob.core.windows.net/sdtest1"
    ...
    while not completed:
        ...
        if transcription.status == "Succeeded":
            ...
            for file_data in _paginate(api, pag_files):
                ...
                my_temp = results.content.decode('utf-8')
                my_temp = my_temp.replace("\n","")
                json_without_slash = json.loads(json.dumps(my_temp, indent = 4))

                current_name = audiofilename.split('/')[1].split('.')[0]
                current_output_json_pth = output_folder_json_pth.joinpath(current_name + '_raw.json')

                with open(current_output_json_pth, 'w', encoding='utf-8') as jsonf:
                        jsonf.write(json_without_slash)
